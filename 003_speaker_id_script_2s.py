...
for i in range(5):
    ...
    result_file = "003_results_2s.jsonl"
    TASK = "speaker_id_2s"
    # %%
    ...
    def preprocess_function_with_clipping(examples):
        speech_list = [speech_file_to_array_fn(path)[0:2*target_sampling_rate] for path in examples[input_column]]
        target_list = [label_to_id(label, label_list) for label in examples[output_column]]
    ...
    train_dataset = train_dataset.map(
        preprocess_function_with_clipping,
        batch_size=100,
        batched=True,
        num_proc=4
    )
    ...
    result = dev_dataset.map(predict, batched=True, batch_size=8)
    y_true = [config.label2id[name] for name in result["Speaker_name"]]
    y_pred = result["predicted"]
    with open(result_file, "a") as f:
        payload = {
